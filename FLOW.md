...
5. Step 3 has automatic repair: if the LLM generates overlapping placements, a simulated annealing algorithm resolves overlaps before counting it as a failed attempt. Board dimensions are auto-sized based on total component area if not specified in requirements. If all LLM rework attempts fail, two fallbacks run: (a) grow the board 20% and re-run SA repair, (b) generate a deterministic grid-based placement on a 30%-larger board and repair it.
...
7. Step 4 (Routing) uses **Freerouting** (external autorouter via Specctra DSN/SES). It auto-downloads on first use and requires Java 17+; if it fails, routing reports the failure rather than falling back to a weaker engine. GND net is excluded from trace routing and connected via copper fill (both layers) with thermal relief and island removal. IPC-2221 trace widths are computed before routing and passed to Freerouting via DSN net classes (preventing current-capacity DRC violations).
...
9. After DRC, a **mandatory approval gate** opens an interactive board viewer in the browser. The user reviews traces, copper fills, per-net stats, and DRC results. Three actions: **Export KiCad** (download `.kicad_pcb` for manual editing), **Import KiCad** (upload edited `.kicad_pcb` to re-import), **Continue to Output** (approve and proceed to Step 6). Pipeline blocks until Continue. Use `--skip-approval` to bypass for batch/CI runs, or `--agent-mode` for vision-based autonomous review (auto-approves on escalation).
10. Step 6 (Output Generation) produces manufacturer-ready files: 8 Gerber layers (copper, silkscreen with stroke font, solder mask, paste, edge cuts), Excellon drill, BOM CSV (with `LCSC Part #` for JLCPCB assembly matching), pick-and-place CSV (with fiducials), bare PCB STEP model, and a ZIP package for upload. Silkscreen text avoids pad/fiducial exclusion zones. Part numbers are resolved deterministically at export (`orchestrator/quoting.py`); `get_fab_quote` adds a board-price estimate plus live LCSC stock/price.
