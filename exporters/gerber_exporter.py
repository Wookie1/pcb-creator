...
# Excellon header: decimal metric coordinates, declared explicitly (the same
# format KiCad writes) so no CAM tool has to infer the digit format.
_DRILL_HEADER = [
    "M48",
    "; DRILL file generated by pcb-creator",
...
def export_drill(
    ...
    for i, drill in enumerate(tools, start=1):
        ...
        for x, y in by_drill[drill]:
            # Explicit decimal coordinates — no implied-digit format for a fab
            # CAM tool to guess (integer "METRIC,TZ" hits are read as 3.3 by
            # some tools and 4.2 by others).
