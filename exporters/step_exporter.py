"""Export PCB as a STEP (ISO 10303-21) file.

Generates a solid 3D model of the board as an extruded polygon, optionally
populated with 3D component models. Supports rectangular and arbitrary
outline shapes.

The STEP file uses the AP214 application protocol and represents shapes
as BREP (Boundary Representation) solids: top face, bottom face, and
edge faces connecting them.

Component models are sourced from LCSC/EasyEDA (via easyeda2kicad) with
parametric box fallbacks for missing models.
"""
...
def export_step_populated(
    ...
    """Export PCB with 3D component models as STEP file.

    Generates the bare board solid plus a parametric 3D box for each
    placed component. If LCSC part numbers are available, attempts to
    fetch real STEP models from EasyEDA (cached locally).

    Falls back to export_step() if parametric_models import fails.

    Args:
        routed: Routed dict containing board dimensions and placements.
        netlist: Netlist dict.
        bom: BOM dict (may contain LCSC part numbers in future).
        output_path: Where to write the .step file.
    ...
    # --- Build board BREP (reuse same logic as export_step) ---
    entities: list[str] = []
    eid = [0]
    ...
    def build_edge_loop(point_ids: list[int], plane_id: int) -> int:
        ...
        for i in range(len(vertex_ids)):
            j = (i + 1) % len(vertex_ids)
            line_dir_id = next_id()
            entities.append(f"#{line_dir_id} = VECTOR('',#{dir_x},1.);")
            line_id = next_id()
            entities.append(f"#{line_id} = LINE('',#{point_ids[i]},#{line_dir_id});")
            edge_id = next_id()
            entities.append(f"#{edge_id} = EDGE_CURVE('',#{vertex_ids[i]},#{vertex_ids[j]},#{line_id},.T.);")
            oe_id = next_id()
            entities.append(f"#{oe_id} = ORIENTED_EDGE('',*,*,#{edge_id},.T.);")
    ...
    for i in range(n):
        j = (i + 1) % n
        side_pts = [point_ids_bot[i], point_ids_bot[j], point_ids_top[j], point_ids_top[i]]
        ax_id = next_id()
        entities.append(f"#{ax_id} = AXIS2_PLACEMENT_3D('',#{side_pts[0]},#{dir_x},#{dir_z_up});")
        side_plane = next_id()
        entities.append(f"#{side_plane} = PLANE('',#{ax_id});")
    ...
    board_solid_id = next_id()
    entities.append(f"#{board_solid_id} = MANIFOLD_SOLID_BREP('{project}_board',#{shell_id});")

    # --- Generate component models ---
    component_solid_ids: list[int] = []
    placements = routed.get("placements", [])
    ...
    for plc in placements:
        ...
        comp_entities, solid_id, new_eid = generate_component_model(
            placement=plc,
        ...
        eid[0] = new_eid
        component_solid_ids.append(solid_id)

    # --- Shape representation with all solids ---
    all_solid_refs = ",".join(ref(sid) for sid in [board_solid_id] + component_solid_ids)
    srep_id = next_id()
    entities.append(f"#{srep_id} = SHAPE_REPRESENTATION('',({all_solid_refs}),#{axis_bot});")

    # Product and shape definition
    prod_id = next_id()
    entities.append(f"#{prod_id} = PRODUCT('{project}','{project} PCB Assembly','',(#{next_id()}));")
    pc_id = eid[0]
    entities.append(f"#{pc_id} = PRODUCT_CONTEXT('',#2,'mechanical');")
    ...
    # --- Write STEP file ---
    entity_block = "\n".join(entities)
    comp_count = len(component_solid_ids)

    step_content = f"""\
ISO-10303-21;
HEADER;
FILE_DESCRIPTION(('PCB Assembly Model - {comp_count} components'),'2;1');
FILE_NAME('{output_path.name}','{now}',('pcb-creator'),(''),
  'pcb-creator 1.0','pcb-creator','');
FILE_SCHEMA(('AUTOMOTIVE_DESIGN'));
ENDSEC;
