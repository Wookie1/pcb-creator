...
| `optimize_placement(project_name, board_width_mm?, board_height_mm?, seed?)` | **Granular.** Deterministic grid placement → repair → fast SA. Synchronous. **Blocks with `unresolved_footprints` if any footprint is unresolved.** Board dims required on first placement (a netlist carries no outline); reused on re-runs. `seed` makes it reproducible. |
