...
**Status: v2 implemented, AUTO-enabled on fine-pitch boards** (`config.escape_fanout` is tri-state: `None` = auto-enable when a part is below `FINE_PITCH_THRESHOLD_MM`; `PCB_ESCAPE_FANOUT=true/false` forces it). `generate_escape_routing` detects single-row/column fine-pitch parts and emits the **full breakout** for every escaping pin — pad → short stub → staggered through-via → a deterministic **onward fanout trace** that threads the gaps between the far via row and exits on a clean release line at the pad pitch — all returned in routed-schema form and fed to Freerouting as `fixed_routing` `(type protect)` wiring on a fresh route. The autorouter therefore starts from a comfortable-pitch grid *clear of the pad field* and never has to fan the group out itself. Unit-tested in `tests/test_escape_router.py`. **The 0.5 mm `CN1` on morgan now routes 0 shorts / 100% (Pi-validated).** Multi-row / quad parts (QFP/QFN) are still skipped (left to the autorouter).

**The root-cause correction (important).** An earlier verdict here held that escape fanout was *neutral* on morgan and the real wall was a 4-layer `plane_layers=1` *stackup* limit. **That was wrong.** The dominant `CN1` failure (~86 DRC errors, mostly `shorting_items`) was a **systematic exporter bug**, not a routing limit: `kicad_exporter` wrote SMD pads with their raw, unrotated `(size w h)`, but **KiCad does not rotate an SMD pad's rectangle with the footprint angle on load** (verified via `pcbnew.GetBoundingBox` — a −90° part's pads report orient 0 and keep their authored w×h in board frame). So `CN1`'s 1.3 mm-long pads stayed oriented along the 0.5 mm-pitch axis and **overlapped their own neighbours → 45 pad-pad shorts** plus the track-pad shorts those caused. Freerouting was fine all along (its DSN image rotates pad shapes); only the KiCad export was corrupt, which is why the shorts looked like an unroutable fine-pitch fanout. Fix: the exporter pre-swaps pad w/h for 90°/270° (mirroring `build_pad_map`); see *Export → Pad rotation*. With that fixed, the escape router's breakout is what makes the connector route cleanly.
