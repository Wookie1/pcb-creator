...
│   └── router.py               # IPC-2221 widths + copper fills/planes + silkscreen
...
│   ├── validate_routing.py     # Schema + trace/via/pad/fill clearance + connectivity + pad-row escape
...
Set `"board": { "layers": 4 }` to request a 4-layer board (F.Cu signal → In1.Cu GND plane → In2.Cu power plane → B.Cu signal). The most-connected non-GND power net is automatically chosen as the inner2 plane (e.g. VCC3V3 over VCC5 if VCC3V3 connects more components).
...
**Every MCP tool that consumes pad geometry must call `_activate_project_lookup(project_name)` first.** The lookup is *module-level state*, so a long-running server that already ran `optimize_placement`/`route_board` has it set and works by accident, while a **cold** server (fresh process, or the first call after a restart) resolves pads through a different tier — silently producing a *different board*. This is not cosmetic: on a clean 4-layer board, a cold `run_drc` reported **24 shorts + 15 clearance + 8 connectivity errors that do not exist**, all of which vanished once the lookup was activated. `route_board`, `run_drc`, `export_kicad`, and `get_board_image` were missing the call (only `design_pcb`/`verify_footprints`/`optimize_placement`/`export_outputs`/`set_component_positions` had it) — an agent that restarted the server between routing and DRC would be told its good board was full of shorts, and would "fix" a phantom.
...
**Functional grouping (`SAConfig.grouping_weight`, `functional_group`).** The `_grouping_cost` term pulls functionally-related parts together (Manhattan distance over a set of affinity pairs), which reduces inter-block net crossings and the congestion the router struggles with most. The pairs come from two sources, unioned in `_build_grouping_pairs`:
- **Shared-net floor (always on):** components sharing ≥2 nets are inferred related. This is the historical behavior and the lower bound — it is always present, so a missing or wrong declared group can never drop the result below it.
- **Declared functional groups (optional):** components the schematic engineer (or an agent) tags with the same `functional_group` label — a short block name like `power`, `mcu`, `usb`, `analog`. Full mesh within each block. This is the *explicit-hierarchy* signal: the LLM/agent knows the real circuit sections, whereas the shared-net heuristic only guesses them from topology. `_read_functional_groups` reads the label from the netlist component's first-class `functional_group` field (falling back to `properties.functional_group`).

The two sources are deduped to one pair each (uniform weight), so they reinforce without double-counting. A **degenerate-group guard** drops any label covering >60% of the board (an "everything is `main`" tag carries no placement signal), falling back to the floor for those parts. The label reaches the netlist from all three netlist-producing paths: the LLM schematic step (`schematic_generate_components` prompt), structured `requirements_json` (the requirements schema carries `functional_group` per component), and the from-scratch builder (`add_component(functional_group=...)`, persisted by `circuit_builder.finalize`). Since the floor is always present, the feature is safe against a mis-tag: worst case is a few spurious low-weight affinity pairs, which costs a little wirelength, never correctness.

**Two-sided placement (`SAConfig.two_sided`).** A layer-flip move (≈15% in optimize, 20% in repair) moves small SMD passives (resistor/capacitor/diode only — never connectors, ICs, LEDs, through-hole, pinned, or keepout parts) to the bottom side. A `bottom_penalty` term keeps flips reluctant on 2-layer boards (the bottom is the router's only escape layer there) but cheap on 4-layer with `plane_layers=1` (inner signal layer frees capacity). Bottom pads are X-mirrored (`dx → -dx`) consistently across `build_pad_map`, the KiCad export, and the Gerber/CPL side fields; layer-conflict checks treat through-hole parts as blocking *both* sides (`_layers_conflict`, `_effective_layer`). Use it to make a dense board *fit*, not to improve routability. **It is opt-in and never inferred from density** — putting parts on the bottom changes assembly cost, so it is the user's call (`board.two_sided` in requirements, or the explicit argument). **Resolution order: explicit argument → requirements → previous placement, and the first source that says anything wins — including `false`.** Setting it back to `false` genuinely turns it off; it used to be a one-way latch (the previous placement was OR'd in, so once any run enabled it the requirements value was ignored forever). The previous placement is still consulted when requirements is *silent*, which is the routing retry loop's path — it re-places without passing the flag and must stay consistent with the route it is retrying. **Enabling it does not guarantee it helps:** it only relieves the flip-eligible share of the board's area, so a board whose area is dominated by connectors, ICs and through-hole parts can be over-full *with both sides in use* — `test_arduino_nano` needed 116% of one side and still failed with 12 of 27 parts flipped. Check the placement's `layer` fields before concluding the flag did anything; `test_r2r_dac` is the eval fixture that actually exercises this path. **Flip guard:** beyond the resistor/capacitor/diode type gate, a flip candidate must have ≤3 pins and a coarse (≥0.8mm) pad pitch — so a high-pin or fine-pitch part *mis-typed* as a passive (e.g. a 30-pin 0.5mm FPC inferred as `capacitor`) can never be sent to the bottom.
...
**Routing engine:** Freerouting, the only engine. (A built-in grid A* router was kept as a 2-layer fallback until it was removed — Freerouting is strictly more capable, and a silent fallback to a weaker engine hid real Freerouting failures behind a worse board.)
...
- **Orchestration** (`optimizers/freerouter.py`): Auto-downloads JAR to `~/.cache/pcb-creator/` on first use. `-mt 1` (single-threaded, avoids clearance bugs); the pass budget and the timeout come from the `effort` level (fast = 5 passes/120s, normal = 20/300s, best = 40/900s + one retry on timeout). **The pass budget is applied twice, on purpose.** `-mp` is *inert* in 2.1.0 headless: it lands in `RouterSettings.maxPasses`, but `BatchAutorouter` bounds its loop with `get_stop_pass_no()` — a private transient field that defaults to 999 and is only ever assigned by the GUI classes, which never run headless (measured: `-mp 2` ran to pass 5). Since 2.1.0 writes the SES only at end-of-passes, an unbounded run on a board it cannot close burns the whole timeout and returns **nothing**. So the launch also exports `FREEROUTING__ROUTER__STOP_PASS_NO`; Freerouting's env-var settings path reflects onto that field (`ReflectionUtil` uses `setAccessible`, and `router` is `routerSettings`' Gson `@SerializedName`), which is what actually stops the loop. `-mp` stays because 2.2.4+ dropped `stop_pass_no` and made `maxPasses` the real bound — together they bound both generations, and the surplus setting is ignored harmlessly. Guarded by `tests/test_freerouter_max_passes.py`. Runs via `Popen` with a stdout reader thread that parses each `Auto-router pass #N … (M unrouted)` line into live progress (plus a ~10s heartbeat). **Graceful timeout:** on hitting the wall clock it SIGTERMs Freerouting and imports a partial SES if one was written, else raises an actionable message — note Freerouting v2.1 only writes the SES at end-of-passes, so a killed mid-pass run usually yields no partial. **JVM heap cap:** the JVM is launched with `-Xmx` (`_default_heap_mb`: `PCB_FREEROUTING_HEAP_MB` if set, else ~55% of RAM clamped to [1024, 6144] MB) — Freerouting 2.x grows memory aggressively (>25 GB observed on a 73-part 4-layer board), so without a cap the OS OOM-kills it into an opaque crash; the cap turns that into a catchable `OutOfMemoryError`. OOM is detected from the JVM output or exit code (−9/137) and reported as a **host memory limit** with the *correct* recovery: raise `PCB_FREEROUTING_HEAP_MB` / use a bigger-RAM host, or reduce what the router must hold (fewer parts, smaller board, power+ground on planes). It explicitly does **not** advise "add a routing layer" — more signal layers make Freerouting use *more* memory, so that worsens OOM (the old message had this backwards). `ensure_java()` also probes common Java paths (`/usr/bin/java`, …) before `PATH`, since the MCP server may run with a restricted environment. **On failure:** if Freerouting raises (timeout-with-no-partial, OOM, crash), `run_routing` logs it and returns an error — there is no second engine to fall back to. The message is framed as a failure of *this run*, not a board limit (it explicitly tells the agent N-layer routing is supported and to retry / `keep_existing`, **not** to reduce layers or enlarge the board), because agents read a bare routing failure on a 4-layer board as "use fewer layers" and abandon perfectly routable designs.
- **Copper fills** (`router.py:apply_copper_fills()`): Standalone function that rebuilds a routing grid from the Freerouting output, marks existing traces/vias, then runs the standard fill algorithm. Removes a plane net (GND on 2-layer; GND + power net on 4-layer) from the unrouted list **only when it is genuinely delivered** — every same-net SMD pad reached the plane through a stitching via; a power-plane pad that found no clear via site is recorded in `routing.unstitched_plane_pads` and the net stays unrouted. **Completion is then reconciled against the authoritative connectivity check** (`validators.validate_routing.incomplete_net_ids` — segment-aware union-find over traces/vias, crediting fill/plane delivery): `unrouted_nets`/`completion_pct` are recomputed from it, so a net the autorouter counted as done but that still has a pad gap is reported unrouted rather than credited. (Completion is pad-level, not the autorouter's net-level count — it is no longer forced to 100%.)

### Post-Routing Processing & Validation
...
**Validation:** 8 checks — schema, trace-to-trace clearance, via clearance, connectivity (union-find), no-shorts, fill shorts (pours/planes painted over foreign copper), trace/via-to-pad clearance, pad-row escape (a 2-row TH package's inter-row channel must not be threaded by wiring that connects to nothing — audit F2: IN2 through the L298N pad rows validated clean structurally while KiCad DRC saw pad shorts + dangling tracks). Traces with an endpoint on a pad of the package are connections, not escapes; inner copper layers are exempt; narrow-gap footprints (pin headers) form no channel. Regression tests in `tests/test_pad_row_escape.py`.
...
5. Add **rescue vias** — find top-layer fill islands disconnected from GND and drop a via to tie each back to GND. On a **2-layer** board the via must land where bottom fill exists (connects through the bottom plane); on a **4-layer** board In1.Cu is a solid GND plane, so a through-via reaches GND directly with no bottom fill needed (`_add_rescue_vias(..., inner_gnd_plane=True)`, set from `inner_plane_count(board) >= 1`) — but the via still lands on the bottom layer, so the candidate cell's bottom must be empty or GND (a foreign signal trace there would be a hard short). Only rescues islands ≥4 cells to avoid saving tiny slivers. (This is the *grid-model* mitigation — for the authoritative guarantee on the exported board see *KiCad Export → GND island stitch*.)
...
- Copper fills → `zone` definitions (board-outline zones on F.Cu/B.Cu + inner planes). The export then **pours** them with `pcbnew` (`fill_zones_pcbnew`), so the saved `.kicad_pcb` ships with real `filled_polygon` geometry — `kicad-cli pcb drc` (which does not pour) sees connected copper instead of a flood of false "unconnected". Best-effort: skipped if no `pcbnew` is reachable (KiCad's GUI auto-pours on open regardless).
...
- **Finding `pcbnew`** (`_kicad_python_candidates`): `pcbnew` ships only with KiCad's own interpreter, not the project venv. The export probes `PCB_KICAD_PYTHON`, then KiCad.app's bundled framework python (`…/Python.framework/Versions/*/bin/python3`, derived from `PCB_KICAD_CLI` or the default install), then `sys.executable` and the system python — without this the macOS pour silently no-op'd and every exported board shipped unpoured.
- **GND island stitch** (4-layer, `stitch_gnd_islands_pcbnew` → `exporters/_gnd_stitch.py`): after the pour, a `pcbnew` pass over the **authoritative poured geometry** finds GND `GetFilledPolysList` regions with no through-via to the inner plane (containment is hole-aware — a via sitting in a clearance void does NOT count as on the island), drops a clearance-checked GND through-via into each (foreign-via clearance uses the annular ring, same-round vias keep mutual drill spacing; KiCad re-pours the In2 antipad around it — no power short), and re-pours. This is the geometry-authoritative guarantee that the grid-model rescue (above) can only approximate; an island so congested that no clearance-legal via site exists can rarely remain (surfaced as `routing.unstitched_gnd_islands`). **Two invariants make the stitch safe to ship:** (1) *never below the board's own rules* — the via size is capped by `board_via_minima` (the same floor written into the `.kicad_pro`), so the pass can't emit the 0.45/0.2 via that failed DRC as `via_drill_min`/`annular_ring` on a 0.6/0.3 board while `routed.json` looked clean; if no legal via fits, the island is left and counted rather than shipping an unmanufacturable hole. (2) *what ships is what passed DRC* — the added vias are harvested back into `routed.json` (`_harvest_stitch_vias`, deduped by position) and the inner planes are **re-cut** around them (`regenerate_inner_planes`), because the Gerbers paint the plane from the stored polygons: without the re-cut those GND vias would sit in solid power-plane copper in the Gerbers (KiCad hides it by re-pouring its own antipads). `stages.run_drc` persists the harvest and then re-runs DRC **once** so the verdict certifies the board that will actually ship.
...
**Check categories:**
- **Electrical** (from `validate_routing.py`): trace clearance, via clearance, connectivity (union-find), no shorts, fill shorts, pad clearance, pad-row escape
...
2. **Two staggered via rows** (alternating distance by pin parity) so Ø0.45 mm escape vias clear each other at the pad pitch. The near-row pins' onward traces thread the gaps *between* the far-row vias (≈0.2 mm clearance) — placed deterministically because the generic autorouter can't reliably hit those channels. **Inter-net escape clearance:** before placing each pin's via, three guards run — via↔via spacing, via↔foreign-net stub/fanout trace, and this pin's stub↔any foreign via (order-independent). If a pin can't be placed clear it's *skipped* and falls to the autorouter (a plane-net pin is still connected by its plane). This prevents the fine-pitch clearance violation between two *protected* escapes (a 5V escape via 0.04 mm from a GND escape stub) that the short-cleanup can't move because neither is re-routable.
...
**Violation-site keepouts + pad-protect (the "5 rounds, same mistake" fix).** Ripping and re-routing a net is useless if Freerouting just re-makes the same choice — an I2C trace squeezed past the adjacent header pad shorted it for *5 consecutive cleanup rounds*. So each cleanup pass now **walls off the violation loci with keepouts** (`parse_cleanup_drc` reads each violation's position; `cleanup_shorts` accumulates a keepout there and threads it into the re-route's `fixed_routing`), forcing the re-routed net clear of where it shorted. It's **net-safe by construction**: only the ripped nets are re-routed (everything else is protected), and for a short to a *pad*, that pad's net is kept **protected** (not ripped) so the keepout sitting on it can't disconnect it — the *foreign* (track/via) net is the one ripped and pushed away. Keepouts accumulate across iterations so a fixed short can't reappear.

**Stitching-via spacing — vias *and* mounting holes (`_filter_via_hole_spacing` + `_mounting_hole_keepouts` in `apply_copper_fills`).** GND/power stitching vias are placed on a coarse grid, but one can still land within the hole-to-hole minimum of a Freerouting routing via, another stitching via, or a **mounting hole**. Two cases observed: two GND vias 0.25mm apart (`hole_to_hole`), and a GND via 0.7mm from a corner-seeded M3 mounting hole (`hole_clearance` + `hole_to_hole` — the corner-seeding parks holes exactly where corner GND fill wants stitching vias). Because stitching vias are *redundant* plane connections, the merge step drops any new stitching via that would sit within `via_drill_mm + HOLE_TO_HOLE_MIN_MM` (0.5mm edge-to-edge) of an existing/kept via **or** within `hole_radius + via_radius + HOLE_TO_VIA_CLEARANCE_MM` of a mounting hole (drill parsed from the `MountingHole_<d>mm` package). Existing routing vias are never dropped. Removes the violation at the source. Unit-tested in `tests/test_via_spacing.py`.

**Dangling-stub cleanup (`_remove_dangling_traces` in `apply_copper_fills`).** Rip-up/reroute leaves trace stubs with a free end (the `track_dangling` DRC warnings). Before pouring fill, any trace with an endpoint not supported by a pad, via, or other trace *of the same net* is removed — iterating so a chain of stubs collapses. It's wrapped in a **connectivity safety net**: `incomplete_net_ids` is checked before/after and the removal is reverted if any net gains a disconnection, so it can never trade a cosmetic warning for a real connectivity error. Vias are left untouched (plane stitching vias connect fills, not traces). Same-net-aware and unit-tested in `tests/test_via_spacing.py`.

**SMD pads connect solid to zones (`exporters/kicad_exporter.py`).** Small SMD pads (0805 GND, …) can't fit KiCad's minimum two thermal-relief spokes, so thermal relief there trips the `starved_thermal` DRC. SMD pads have little thermal mass, so they're exported with `(zone_connect 2)` — a **solid** zone connection — while through-hole pads keep thermal relief (`zone_connect 1`): TH pads are large enough for spokes and benefit from relief for rework. This eliminates `starved_thermal` on SMD ground pads.

### Future Enhancements

- **Manufacturer quoting (Step 7)**: Auto-submit BOM, Gerbers, assembly files to manufacturer APIs (JLCPCB, PCBWay, OSH Park) for fabrication + assembly quotes. Present comparison to user. Steps 5-6 produce submission-ready files in manufacturer-expected formats. Blocked on: vendor API availability or TOS review for agent accounts + Playwright. **First rung shipped:** the BOM now carries orderable part numbers (`lcsc`/`mpn`/`manufacturer` — curated jellybean table + cache + LLM suggestion, resolved at export in `orchestrator/quoting.py`), the BOM CSV has the `LCSC Part #` column JLCPCB's assembly matcher keys on, and `get_fab_quote` returns a deterministic board-price estimate (marked `estimate: true`, table overridable via `PCB_PRICE_TABLE`) plus live LCSC stock/price per part id with an MPN cross-check. `set_part_number` closes the loop for parts the resolver can't fill: the agent looks the part up and records it, and the id is cached so it auto-resolves everywhere afterward.
- **Differential pairs (USB D+/D− and beyond)**: The pipeline routes diff pairs as unrelated single-ended nets today (only the exported `.kicad_pro` carries `diff_pair_*` default rules — `kicad_exporter.py`). Fine in practice for USB-FS; wrong for USB-HS/RMII/LVDS. Design sketch: detect pairs by net-name convention (`D+`/`D−`, `_P`/`_N` suffixes) or an explicit net-class tag; pre-route each pair with an escape-router-style deterministic paired track (controlled gap, matched length) handed to Freerouting as protected wiring — Freerouting has no native diff-pair mode, so the pair must be drawn before it runs; add a DRC check for intra-pair skew and gap. Until then the honest gap is documented here.
...
- ~~**Code-enforced approval gate**~~ ✅ **Implemented** — see the "Route-failure escalation ladder" note: `optimize_placement` / `set_component_positions` now enforce `approved=True` in code for 2→4 layer promotion and board enlargement.
...
| `build_circuit(project_name, description, board_width_mm, board_height_mm, components, nets, no_connect?, layers?, overwrite?)` | **Build-from-scratch, bulk.** `create_circuit` + every `add_component` + every `connect_pins` + `finalize_circuit` in one call, for an agent that already knows the circuit. Same validation per item (it calls the same tools); attempts every item rather than aborting on the first bad one, and a failure returns `failed[]` (per item: error + its own remediation) with `added`/`connected` counts — the applied items stay in the draft for targeted repair. |
...
| `set_part_number(project_name, designator, lcsc?, mpn?, manufacturer?)` | **Granular.** The write half of the quote loop: record an orderable part id on one BOM line (designator matches its group, e.g. "D2" updates the "D1, D2, D3" line; the BOM is synthesized from the netlist if absent). Persists to `<project>_bom.json`, flows into the BOM CSV's `LCSC Part #`/`MPN` columns, and is cached by `type:value:package` (source `agent`) so the part auto-resolves in future projects. |
...
| `get_board_image(project_name, width?)` | Render the board as a real MCP **image content block** the calling model can see — the routed board, or the **placement** when the board is not routed yet (`stage` says which), so layout is reviewable *before* paying for a route. `width` defaults to 1024 and is clamped to 256–2048: the old base64-in-JSON return at `width=2048` cost ~47k tokens of text the model could not actually look at. |
| `revert_board(project_name)` | Undo the last `optimize_placement` / `route_board`, restoring the previous placement **and** routed board together (both are snapshotted at the start of every such run, so the restored pair is always coherent). One step of history, one slot; refuses while a route is running, and drops the now-stale DRC report. The answer to a nondeterministic re-route landing worse than the board it replaced. |
...
**Eval board suite (`test/requirements/`, driven by `scripts/eval_boards.py`).** The regression gate for placement/router tuning: every board is compiled through the incremental circuit builder (no LLM), placed, routed and DRC'd, and the scoreboard lands in `scripts/eval_output/`. Two boards carry a specific job and should not be casually resized:

- **`test_r2r_dac`** is the **two-sided fixture** — an 8-bit R-2R ladder, deliberately passive-dominated so a meaningful share of its area is flip-eligible, and sized so the placer genuinely puts parts on the bottom. Guarded by `TestTwoSidedEvalFixture` in `tests/test_two_sided.py`, because the way a two-sided fixture dies is silent: it drifts into fitting entirely on top and still looks healthy on the scoreboard. That is exactly what `test_arduino_nano` did while it carried `two_sided: true`.
- **`test_arduino_nano`** is the **hard-routing** board (27 parts, 38 nets, two 15-pin through-hole headers blocking both layers of a 2-layer board, fanning into a TQFP-32). Freerouting does not close it — expect a partial route, not a PASS. It was 45x18mm until it turned out to be infeasible at that size once its headers resolved at the correct 2.54mm pitch; it is 50x20mm now, which is the smallest size measured to place. Note **more passes make this board worse**, not better (measured 20 passes → 37.8%, 40 passes → 13.5%): Freerouting's optimizer rips up more than it re-routes here.

Treat single-run DRC-error counts from this suite as noise — Freerouting is not run-to-run deterministic (the same board yields 0/3/3 errors across identical runs). Completion percentage is the stable signal.
