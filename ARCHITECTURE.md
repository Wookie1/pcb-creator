...
- **Manufacturer quoting (Step 7)**: Auto-submit BOM, Gerbers, assembly files to manufacturer APIs (JLCPCB, PCBWay, OSH Park) for fabrication + assembly quotes. Present comparison to user. Steps 5-6 produce submission-ready files in manufacturer-expected formats. Blocked on: vendor API availability or TOS review for agent accounts + Playwright. **First rung shipped:** the BOM now carries orderable part numbers (`lcsc`/`mpn`/`manufacturer` — curated jellybean table + cache + LLM suggestion, resolved at export in `orchestrator/quoting.py`), the BOM CSV has the `LCSC Part #` column JLCPCB's assembly matcher keys on, and `get_fab_quote` returns a deterministic board-price estimate (marked `estimate: true`, table overridable via `PCB_PRICE_TABLE`) plus live LCSC stock/price per part id with an MPN cross-check.
